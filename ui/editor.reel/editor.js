...
exports.Editor = Montage.create(Component, {
    ...
    flowNeedsUpdate: {
        set: function (bool) {
            if (this._timeoutFlowUpdateID) {
                clearTimeout(this._timeoutFlowUpdateID);
                this._timeoutFlowUpdateID = null;
            }

            if (!!bool) {
                var self = this;
                this._flowNeedsUpdate = true;

                this._timeoutFlowUpdateID = setTimeout(function () {
                    if (self._flowNeedsUpdate) {
                        self.convertShapeToFlow();
                        self._flowNeedsUpdate = false;
                    }
                }, 0);
            } else {
                this._flowNeedsUpdate = false;
            }
        },
        get: function () {
            return this._flowNeedsUpdate;
        }
    },
    ...
    convertFlowToShape: {
        value: function () {
            ...
            if (metadata && metadata.flowEditorVersion <= this.flowEditorVersion && metadata.shapes) {
                var shapesLength = metadata.shapes.length;

                for (i = 0; i < shapesLength; i++) {
                    iShape = metadata.shapes[i];

                    if (typeof iShape.pathIndex !== "undefined" && iShape.type === "FlowHelix") {
                ...
                if (!specialPaths[j]) {
                    ...
                    for (i = 0; i < spline.knots.length; i++) {
                        if (i >= canvasSpline.children.length) {
                            ...
                            for (n = i; n < i + 2; n++) {
                                var canvasSplineChild = canvasSpline.children[n];

                                if (canvasSplineChild) {
                                    if (canvasSplineChild.knot && spline.knots[n].knotPosition) {
                                        canvasSplineChild.knot.x = spline.knots[n].knotPosition[0];
                                        canvasSplineChild.knot.y = spline.knots[n].knotPosition[1];
                                        canvasSplineChild.knot.z = spline.knots[n].knotPosition[2];
                                    }

                                    if (canvasSplineChild.nextHandler && spline.knots[n].nextHandlerPosition) {
                                        canvasSplineChild.nextHandler.x = spline.knots[n].nextHandlerPosition[0];
                                        canvasSplineChild.nextHandler.y = spline.knots[n].nextHandlerPosition[1];
                                        canvasSplineChild.nextHandler.z = spline.knots[n].nextHandlerPosition[2];
                                    }

                                    if (canvasSplineChild.previousHandler && spline.knots[n].previousHandlerPosition) {
                                        canvasSplineChild.previousHandler.x = spline.knots[n].previousHandlerPosition[0];
                                        canvasSplineChild.previousHandler.y = spline.knots[n].previousHandlerPosition[1];
                                        canvasSplineChild.previousHandler.z = spline.knots[n].previousHandlerPosition[2];
                                    }

                                    if (canvasSplineChild.knot) {
                                        for (k in spline.units) {
                                            if (typeof spline.knots[n][k] !== "undefined") {
                                                canvasSplineChild.knot[k] = spline.knots[n][k];
                                            }
                                        }

                                        if (typeof spline.knots[n].previousDensity !== "undefined") {
                                            canvasSplineChild.knot.density = spline.knots[n].previousDensity;
                                        }
                                    }
                                }
                            }
                    ...
                    if (specialPaths[j].type === "FlowHelix") {
            ...
            if (!this.viewport.scene) {
                var self = this,
                    updated = function () {self.flowNeedsUpdate = true;};
